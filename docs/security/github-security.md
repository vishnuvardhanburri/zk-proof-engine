...
| **6** | **CodeQL Static Analysis** | `CONFIGURED` | Configured in `.github/workflows/codeql.yml` targeting `javascript-typescript` and `solidity` (`build-mode: none`) with immutable SHA pinning. |
...
3. Check **Require status checks to pass before merging**:
   ...
   - Require status check: `Analyze (solidity)` (`CodeQL`)
